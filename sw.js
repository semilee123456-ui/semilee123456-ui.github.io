...
//
// odds-data.js(확률체감 탭 전용, 492KB)·faq-panel-data.js(FAQ 탭 전용, 2026-08-15 분리,
// 255KB)는 이미 자체 쿼리스트링 캐시버스팅(?v=YYYYMMDD, 손으로 버전을 올리는 방식)을 쓰고
// 있어서, 이 서비스 워커가 그 파일을 가로채거나 캐싱하면 두 캐싱 메커니즘이 충돌할 위험이
// 있음 — 그래서 아래 APP_SHELL_PATHS에 이 파일들은 절대 넣지 않고, 애초에 이 파일들에 대한
// 요청은 fetch 핸들러에서 걸러져서(allowlist에 없으므로) 서비스 워커를 아예 거치지 않고
// 브라우저 기본 동작으로 그대로 흘러감.
...
const CACHE_NAME = 'chamtax-shell-v100';

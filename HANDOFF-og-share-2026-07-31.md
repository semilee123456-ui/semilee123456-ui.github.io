...
> ## ⚠️ 이 파일은 더 이상 최신 상태가 아님 — `HANDOFF.md`의 "작업 이력" 참고
> 이 파일은 원래 별도 topic-partN 파일을 만들지 말라는 `HANDOFF.md`의 규칙과 어긋나게(당시
> 세션이 토큰 소진으로 급하게) 만들어진 것. 아래에 정리된 "카카오톡 미리보기 안 뜨는 문제"는
> **2026-07-31 `claude/progress-checkpoint-pbf93h` 세션이 진짜 원인(workers.dev 공유
> 서브도메인이 링크 미리보기 봇에게 Cloudflare 자체 홍보 페이지를 보여주는 문제)을 찾아서
> 커스텀 도메인(`og.chamtax.com`)으로 해결함 — 자세한 경위는 `HANDOFF.md` "작업 이력"의
> 해당 날짜 세션(PR #13/#14) 항목 참고, 실제 카카오톡 재검증은 아직 다음 세션 몫으로 남아있음.**
> 이 파일 아래 내용은 그 이전까지의 진단 히스토리(스크립트 캐시 버전 문제, 한글 폰트 tofu
> 깨짐 등 — 전부 해결됨)라 배경이 필요할 때만 참고하고, 앞으로 이 프로젝트의 새 handoff
> 내용은 전부 `HANDOFF.md` 한 파일에만 추가할 것(이 파일처럼 새 파일을 또 만들지 말 것).
...
## 2026-07-31 세 번째 진단 라운드 — 실제 사용자 카카오톡 스크린샷 2장으로 진짜 원인 2개 찾음

사용자가 직접 카카오톡 공유 스크린샷을 두 번 보내줘서(이 세션이 실시간으로 확인) 위 "캐시" 가설과는
완전히 다른, 훨씬 구체적인 원인 2개를 실제로 찾았음 — 순서대로:

1. **`script.js` 캐시버스팅 버전을 안 올려서, OG 공유 기능 자체가 없는 옛날 코드가 폰에 캐시돼있었음.**
   증상: "이 결과 공유하기"를 눌러도 카카오 미리보기가 `index.html`의 **고정 기본 og:title/description**
   (사이트 홈 화면 소개 문구)과 완전히 똑같이 나옴 — Worker가 아예 관여를 안 한 상태.
   원인: `og-share-worker` 기능이 `script.js`에 추가된 뒤에도, 그리고 이 세션이 캐시버스팅
   파라미터(`t=`)를 추가한 뒤에도, `index.html`의 `<script src="script.js?v=20260731">` **버전
   문자열을 한 번도 안 올림** — 예전에 사이트를 방문해서 구버전 `script.js`를 캐싱해둔 폰은
   `OG_SHARE_WORKER_BASE`가 없던 시절 코드를 계속 실행하고 있었음(2026-07-31 초반에 있었던
   국가/통화 select 중복 버그와 정확히 같은 유형의 실수 — `HANDOFF.md`의 "GitHub push 정상화"
   섹션 근처 참고). **`?v=20260731-3`으로 올려서 해결(커밋으로 반영, PR #4 병합 완료)**.
2. **버전 올린 뒤 재테스트 → 텍스트(제목/설명/금액)는 정확히 나왔는데, 카드 이미지 안 한글이
   전부 네모(tofu)로 깨짐.** 숫자(3,136)와 %(-46%)는 정상 렌더링됐지만 "억원", "한국 거주자
   기준", "일시불 실수령액" 등 한글 부분이 전부 □로 나옴 — `workers-og`(satori 기반)가 기본
   내장 폰트로 라틴 문자만 지원하고 한글 글리프가 아예 없어서 생기는, 이 라이브러리를 쓸 때
   흔히 겪는 문제. **조치**: `og-share-worker/src/index.js`에 `fetchKoreanFontSubset()` 추가 —
   카드에 실제 쓰이는 글자만(정적 라벨 + 그때그때 다른 finalText/beforeText/country) Google
   Fonts CSS2 API에서 Noto Sans KR 서브셋으로 받아와서(`&text=` 파라미터로 필요한 글자만
   요청, TTF 포맷을 받으려고 구형 User-Agent 사용 — woff2는 satori가 못 읽음) `ImageResponse`의
   `fonts` 옵션에 넘김. `node --check`(ESM) 통과 확인.

**이 폰트 수정은 배포된 Worker에서 실제로 검증 못 함** — 이 샌드박스는 외부 네트워크(WebFetch/curl
둘 다)가 막혀있어서 `fonts.googleapis.com`이나 workers.dev에 직접 요청을 못 보냄. 로직은
`@vercel/og`/`workers-og` 생태계의 표준적인 CJK 대응 패턴을 따랐지만(Google Fonts CSS2 API
`&text=` 서브셋 + 구형 UA로 TTF 유도), **실제로 Cloudflare에서 Google Fonts로 나가는 아웃바운드
요청이 막혀있지 않은지, 그리고 satori가 정확히 이 포맷/이 옵션 형태를 기대하는 게 맞는지는
다음 세션(또는 사용자의 실제 재테스트)이 확인해야 함.** `og-share-worker/`는 Cloudflare
GitHub 연동으로 자동 재배포되므로, 이 커밋이 push되면 별도 조치 없이 곧 반영됨.

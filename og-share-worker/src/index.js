...
const LANG_FONT_FAMILY = {
  ...
  // 카드가 HTTP 200에 본문 0바이트(완전히 빈 이미지)로 깨지는 걸 재현함. **1차 시도로 둘 다
  // Noto Sans Arabic으로 바꿨는데, 배포 후 재검증해보니 우르두어는 고쳐졌지만 아랍어는 여전히
  // 같은 문제("ل+ي+ا" 연속, 특히 "مليار")가 재현됨** — 즉 원인이 단순히 "서예체 폰트"가
  // 아니라, satori(이 Worker의 렌더링 엔진) 자체가 리가처/RTL을 공식적으로 지원 안 해서
  // (vercel/satori 이슈 트래커에 명시됨) 아랍어 문자 결합(joining) 처리 중 이 특정 글자
  // 조합에서 폰트와 무관하게 죽는 것으로 보임(같은 글자를 기본 'Noto Sans'로 그리면 안
  // 죽지만, 그 폰트엔 애초에 아랍 문자 글리프가 없어서 전부 tofu로 나와 실사용 불가). 아랍어만
  // 산세리프 계열 중 리가처 구조가 더 단순한 Cairo로 한 번 더 교체해서 재검증 중 — 이것도
  // 안 되면 폰트 교체로는 해결 불가하다는 뜻이라 다른 접근(예: 위험 조합을 서버에서 감지해
  // 안전한 대체 문구로 치환)이 필요함, 다음 세션 참고. 우르두어는 Noto Sans Arabic 유지(정상
  // 확인됨). 사이트 본문(styles.css)의 폰트는 안 건드림(브라우저는 이 문제가 없음, 이 Worker만의
  // 문제).
  ar: 'Cairo',
  ur: 'Noto Sans Arabic',

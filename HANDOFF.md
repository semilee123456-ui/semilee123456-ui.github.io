...
- **공유카드(OG 이미지) 로고가 옛날 단순 버전 — 기본 카드는 해결, 페이지별 78개는 미해결**:
  ...
  **해결됨(2026-08-05 후속 세션)**: 원본 라스터 이미지를 Playwright 캔버스에 그린 뒤,
  로고+"참택스" 텍스트가 있던 좌상단 영역만(픽셀 스캔으로 정확한 bbox 찾음, x:55~200,
  y:33~97) 배경색(`rgb(244,245,247)`)으로 마스킹하고 그 위에 index.html의 최신 마스코트
  SVG(볼터치·눈 하이라이트 포함)와 동일한 "참택스" 텍스트를 같은 위치·크기·색상으로
  다시 그려서 재합성 — 카드의 나머지 부분(헤드라인·통계 카드·CTA 등)은 원본 픽셀 그대로
  유지됨. 결과물을 실제로 눈으로 대조 확인함(스크린샷 비교), 파일 크기 1200×630 유지
  확인.
  **(2) 페이지별 전용 카드 78개(`og-image-hook-*.png`)는 여전히 미해결** — 라스터
  이미지라 HTML처럼 일괄 텍스트 치환이 안 되고, 각 이미지에 서로 다른 제목/문구가 이미
  박혀있어서 하나씩 원본 문구를 확인하며 재생성해야 함(9개 언어는 폰트 조달 파이프라인도
  필요, 위 2026-08-04 항목 참고) — 로고 아이콘 하나 바꾸자고 하기엔 범위가 커서 사용자와
  먼저 "정말 78개 다 할지"부터 재확인할 것.

...
- **"외국인 검색 노출, Claude가 할 수 있는 건 다 해달라"는 요청 처리 (2026-07-24)**: 네이버/
  ...
  - ~~**검토는 했지만 실행 안 한 것 — 언어별(진짜 26개 다른) og:image**~~ — **완료됨
    ...
    항목 참고.
- **공유카드(OG 이미지) 로고가 옛날 단순 버전 — 나중에 손볼 것(2026-08-05 발견, 미해결)**:
  ...
  78개(`og-image-hook-*.png`, 바로 위 항목 참고)는 전부 옛날 단순 버전 로고(볼터치·눈
  하이라이트 없는 outline만)를 그대로 쓰고 있어서 아직 안 고쳤음** — 사용자가 직접
  확인 요청해서 발견, "나중에 하는 걸로" 미루기로 합의함. 라스터 이미지라 HTML처럼
  일괄 텍스트 치환이 안 되고, 특히 페이지별 78개는 각 이미지에 서로 다른 제목/문구가
  이미 박혀있어서 하나씩 원본 문구를 확인하며 재생성해야 함 — 로고 아이콘 하나 바꾸자고
  하기엔 리스크(오타·레이아웃 틀어짐) 대비 효용이 낮다고 판단해 보류. **재개할 때는**:
  (1) `og-image-hook.png`(기본 카드, 1개)부터 — 이미 2026-08-05 세션이 `lottery-prize-tiers.html`
  전용 카드를 만들 때 쓴 방식(Playwright로 HTML/CSS를 1200×630 스크린샷) 그대로 재사용
  가능, 현재 문구("20억 달러 당첨되면, 실수령액은 얼마일까요?" 등)를 그대로 유지하고
  로고 SVG만 index.html의 최신 버전(위 커밋 `955e054`의 아이콘 마크업 참고)으로 교체하면
  됨. (2) 78개 페이지별 카드는 위 2026-08-04 항목이 언급한 폰트 조달 파이프라인(9개
  언어 폰트 패키지)이 필요해서 범위가 훨씬 큼 — 사용자와 먼저 "정말 78개 다 할지"부터
  재확인할 것.

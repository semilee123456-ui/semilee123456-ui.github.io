...
- **"외국인 검색 노출, Claude가 할 수 있는 건 다 해달라"는 요청 처리 (2026-07-24)**: 네이버/
  ...
  - ~~**검토는 했지만 실행 안 한 것 — 언어별(진짜 26개 다른) og:image**~~ — **완료됨
    ...
    항목 참고.
- **공유카드(OG 이미지) 로고가 옛날 단순 버전 — 기본 카드는 해결, 페이지별 78개는 미해결**:
  2026-08-05 세션에서 index.html 네비게이션의 마스코트 로고(볼터치·눈 하이라이트 포함,
  `var(--card)`/`var(--teal)` 테마 변수 사용)와 90개 콘텐츠 페이지 네비게이션을 전부
  통일했는데(커밋 `955e054`), `og-image-hook.png`(기본 공유카드)와 페이지별 전용 카드
  78개(`og-image-hook-*.png`)는 전부 옛날 단순 버전 로고(볼터치·눈 하이라이트 없는
  outline만)를 그대로 쓰고 있던 게 발견됨.
  ~~**(1) `og-image-hook.png`(기본 카드, index.html 등 11개 페이지가 참조)**~~ —
  **해결됨(2026-08-05)**: 원본 라스터 이미지를 Playwright 캔버스에 그린 뒤, 로고+브랜드
  텍스트가 있던 좌상단 영역만 픽셀 스캔으로 마스킹하고 최신 마스코트 SVG로 재합성 —
  카드의 나머지 부분(헤드라인·통계 카드·CTA 등)은 원본 그대로 유지. **같은 시각 다른
  세션(`session_0176WPs9mTAXF62XcMbUKaf3`)이 독립적으로 동일한 파일을 같은 방식으로
  고쳐 18분 먼저 `main`에 병합함** — 두 세션이 서로 모르고 같은 항목에 손을 댄 것,
  병합 시 바이너리 충돌 나서 이미 배포된 그쪽 결과물을 채택(둘 다 육안상 동등한 품질).
  **다음 세션 참고**: 이 문서의 "알려진 미해결 항목"이 최신 상태라고 100% 믿지 말 것 —
  여러 세션이 겹칠 수 있으니 작업 시작 전 `git fetch origin main`으로 실제 최신 커밋
  로그를 한 번 확인하는 습관이 이런 중복 작업을 줄여줌.
  ~~**(2) 페이지별 전용 카드 78개(`og-image-hook-*.png`)**~~ — **해결됨(2026-08-05
  같은 세션)**: 각 이미지가 라스터라 일괄 텍스트 치환은 안 되지만, 로고 영역만 바꾸면
  되므로 전체 재생성 없이 (1)과 같은 마스킹 방식을 78개에 일괄 적용— 로고 자체는 어느
  페이지든 "ChamTax"(라틴 워드마크, 한국어 "참택스" 아님) 고정이라 언어별 폰트 조달이
  전혀 필요 없었음(애초에 "9개 언어 폰트 필요"라던 우려는 전체-카드 재생성을 가정한
  낡은 판단이었음). 각 파일의 로고 위치를 자동 검출하는 스크립트를 짬(배경색을 최빈값
  샘플링으로 찾고, 로고가 있는 첫 row-band를 gap-tolerance로 분리한 뒤, 그 안에서
  아이콘 특유의 정사각형에 가까운 비율(37×34px, aspect 0.8~1.4)을 가진 서브블록을
  찾아 아이콘으로, 그다음 서브블록을 텍스트로 지목 — RTL 페이지(아랍어·우르두어)는
  로고가 화면 우측에 있는 것까지 자동으로 처리됨). 78개 중 77개는 완전 자동, 1개
  (`og-image-hook-basics-ur.png`)만 좌측 통계 카드가 로고와 같은 y범위에 겹쳐 있어
  자동 검출이 실패해서 좌표를 수동 확인 후 적용. 아이콘 렌더 크기(43px→ink 37×34px)와
  텍스트 폰트 크기(높이 기반 역산, 기준값 18px→13px/22px→16px 보정식)는 별도 오프스크린
  캘리브레이션으로 확정. 78개 전부 1200×630 크기 유지 확인, LTR/RTL·라틴/한자/태국어/
  미얀마어/우르두어 등 스크립트를 골고루 샘플링해 육안 검수, `broken_link_audit`·
  `fact_consistency_audit` 재실행 이슈 0건. **이걸로 OG 카드 로고 불일치 항목은
  (1)(2) 전부 완전히 종결.**
...
- **2026-08-05 신규 — "극단적 자동화 마케팅"(AI 자율 홍보 파이프라인) 제안, 대부분 거절함**:
  ...
  **아직 착수 전, 사용자 확인 대기 중.** 다시 이 종류 제안이 오면 위 판단 기준(플랫폼 ToS
  위반 여부, 이미 거절한 방침과의 충돌 여부, 실제 계정·비용 필요 여부)으로 먼저 걸러낼 것.

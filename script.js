...
const TAX_MODEL = {
  ...
  la_resident: {
    // ⚠️ 라오스 신설 소득세법(No.88/NA, 2026년 7월 시행) — 복권 당첨소득(1천만 킵 초과분)에 5%
...
const STATE_TAX_RATES = {
  ...
  ME:  { label: '메인', labelEn: 'Maine', labelZh: '缅因', rate: 0.0915 }, // 2026년부터 소득 100만불(개인)·150만불(부부합산) 초과분에 2%p 밀리어네어 서차지 추가 시행(7.15%→9.15%)
...
function calcTakeHome(amount, country, stateCode){
  if (country === 'us') {
    ...
    const ftcCreditWon = Math.min(usWithholdingWon, vnCalculatedTaxWon); // FTC 공제액(국내법 제26조2항 기준, 한도 내 상계)
...
// 인트로 배너의 언어/국가 드롭다운(22개 버튼 그리드를 압축한 것) — "이동" 버튼 클릭 시 호출됨.
// value가 "href:"로 시작하면 해당 국가 전용 SEO 페이지로 이동, 아니면 setLanguage 후 계산기로 스크롤.
...
function updateSideBySide(eok, stateCode){
  ...
  // 표시 금액(formatWon 결과)이 같은 나라들은 카드 하나로 묶어서 국기만 나란히 보여줌
  const groups = [];
  implementedRows.forEach(row => {
    const displayAmt = formatWon(row.result.final);
    const last = groups[groups.length - 1];
    if (last && last.displayAmt === displayAmt) {
      last.rows.push(row);
    } else {
      groups.push({ displayAmt, rows: [row] });
    }
  });
  ...
  if (showMoreBtn) {
    if (allCards.length > SIDE_VISIBLE_COUNT) {
      ...
      // 카드 하나가 여러 나라를 묶은 그룹일 수 있어서, "더보기" 남은 개수는 카드 수가 아니라
      // 숨겨진 카드들 안의 실제 국가 수 합계로 계산 — 그래야 "3개국 더보기"인데 펼치면 5개국이
      // 나오는 식의 숫자 불일치가 생기지 않음
      const hiddenCountryCount = allCards.slice(SIDE_VISIBLE_COUNT)
        .reduce((sum, card) => sum + (parseInt(card.dataset.countryCount, 10) || 1), 0);
      showMoreBtn.textContent = pickLang(`${hiddenCountryCount}개국 더 보기 ▾`, `Show ${hiddenCountryCount} more ▾`, `再显示${hiddenCountryCount}个国家 ▾`, `Xem thêm ${hiddenCountryCount} nước ▾`, `ดูเพิ่มอีก ${hiddenCountryCount} ประเทศ ▾`, `Показать ещё ${hiddenCountryCount} стран ▾`);
    } else {

...
document.addEventListener('DOMContentLoaded', () => { applyJackpotData(); runCountUps(); updateHomeCalc(100000000); updateCalc(); initJackpotCardAmt(); updateDrawCountdown(); fetchLiveExchangeRate(); });
...
if (document.fonts && document.fonts.ready) {
  document.fonts.ready.then(() => {
    ['home-final-amt', 'finalAmt'].forEach(id => {
      const el = document.getElementById(id);
      if (el && el.textContent && el.textContent !== '-') fitAmountFontSize(el);
    });
  });
}
...
function onHomeAmountTyped(){
  const millions = parseMillionsInput(document.getElementById('homeAmountInput').value);
  const slider = document.getElementById('homeAmountSlider');
  ...
  slider.min = Math.min(10, Math.round(millions)); // $10M 미만 입력 시 슬라이더 하한도 같이 낮춰 불일치 방지
  slider.value = Math.round(millions);
  updateHomeCalc(millions > 0 ? millions * 1000000 : undefined);
}
